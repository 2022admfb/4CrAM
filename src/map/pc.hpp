...
struct map_session_data {
	...
	struct s_state {
		...
		unsigned int Move : 1; // Move
		unsigned int bCanAttack : 1; // Move
		int Set_Hp_Auto = 1; // Hp
		int Set_Sp_Auto = 1; // Sp
		int Set_Flywing = 1; // Wing
...
#define pcdb_checkid_sub(class_) ( \
	...
	( (class_) >= JOB_WOLF_WINDHAWK			&& (class_) <= JOB_IMPERIAL_GUARD_CHICKEN ) \
)
