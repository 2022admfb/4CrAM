...
static bool itemdb_read_noequip(char* str[], int columns, int current) {
	...
	if( flag & 64 ){
		id->flag.dropEffect = DROPEFFECT_CLIENT;
	}else if( flag & 128 ){
		id->flag.dropEffect = DROPEFFECT_BLUE_PILLAR;
	}else if( flag & 256 ){
		id->flag.dropEffect = DROPEFFECT_YELLOW_PILLAR;
	}else if( flag & 512 ){
		id->flag.dropEffect = DROPEFFECT_PURPLE_PILLAR;
	}else if( flag & 1024 ){
		id->flag.dropEffect = DROPEFFECT_ORANGE_PILLAR;
	}else if( flag & 2048 ){
		id->flag.dropEffect = DROPEFFECT_RED_PILLAR;
	}
