...
ACMD_FUNC(jobchange)
{
    ...
    if (!message || !*message || sscanf(message, "%11d %11d", &job, &upper) < 1) {
		...
		// High Jobs, Babys and Third // High Jobs, Babys, Third, and Forth
	...
	if (job == JOB_KNIGHT2 || job == JOB_CRUSADER2 || job == JOB_WEDDING || job == JOB_XMAS || job == JOB_SUMMER || job == JOB_HANBOK || job == JOB_OKTOBERFEST
		...
		|| (job >= JOB_RUNE_KNIGHT2 && job <= JOB_MECHANIC_T2) || (job >= JOB_BABY_RUNE_KNIGHT2 && job <= JOB_BABY_MECHANIC2) || job == JOB_BABY_STAR_GLADIATOR2 || job == JOB_STAR_EMPEROR2 || job == JOB_BABY_STAR_EMPEROR2 ||
...
void autoattack_motion(struct map_session_data* sd)
{
	...
	int Item_AGI;
	int Item_ConPotion;
	int Item_AwakePotion;
	int Item_BerserkPotion;

	if (pc_isdead(sd) || !sd->state.autoattack) return;

	for (a = 0 * MAX_HOTKEYS; a < MAX_HOTKEYS_PACKET; a++) {
		//Calculate key
		p.hotkey[a].isSkill = sd->status.hotkeys[a].type;
		p.hotkey[a].id = sd->status.hotkeys[a].id;
		p.hotkey[a].count = sd->status.hotkeys[a].lv;
		//item
		Item_Hp = pc_search_inventory(sd, p.hotkey[9].id);
		Item_Sp = pc_search_inventory(sd, p.hotkey[10].id);
		Item_flywing = pc_search_inventory(sd,p.hotkey[8].id);
		Item_Bless = pc_search_inventory(sd, p.hotkey[11].id);
		Item_AGI = pc_search_inventory(sd, p.hotkey[12].id);
		Item_ConPotion = pc_search_inventory(sd, p.hotkey[13].id);
		Item_AwakePotion = pc_search_inventory(sd, p.hotkey[14].id);
		Item_BerserkPotion = pc_search_inventory(sd, p.hotkey[15].id);
	}
	for (i = 0; i <= 9; i++)
	{
		MaxHp = sd->status.max_hp;
		Hp =sd->status.hp;
		MaxSp = sd->status.max_sp;
		Sp = sd->status.sp;

		// bless scroll and agi
		if (!sd->sc.data[SC_BLESSING])
		{
			if (p.hotkey[11].isSkill == 0) {
				pc_useitem(sd, Item_Bless);
			}
		}
		if (!sd->sc.data[SC_INCREASEAGI])
		{
			if (p.hotkey[12].isSkill == 0) {
				pc_useitem(sd, Item_AGI);
			}
		}

		//Concen Potion
		if (!sd->sc.data[SC_ASPDPOTION0])
		{
			if (p.hotkey[13].isSkill == 0) {
				pc_useitem(sd, Item_ConPotion);
			}
		}
		if (!sd->sc.data[SC_ASPDPOTION1])
		{
			if (p.hotkey[14].isSkill == 0) {
				pc_useitem(sd, Item_AwakePotion);
			}
		}
		if (!sd->sc.data[SC_ASPDPOTION2])
		{
			if (p.hotkey[15].isSkill == 0) {
				pc_useitem(sd, Item_BerserkPotion);
			}
		}

		//item
		if(Hp *100 / MaxHp <= sd->state.Set_Hp_Auto)
		{
			//while (Hp * 100 / MaxHp <= sd->state.Set_Hp_Auto)
			//{
				if (p.hotkey[9].isSkill == 0) {
					pc_useitem(sd, Item_Hp);
				}
			//}
		}
		if (Sp *100 / MaxSp <= sd->state.Set_Sp_Auto)
		{
			//while (Sp * 100 / MaxSp <= sd->state.Set_Sp_Auto)
			//{
				if (p.hotkey[10].isSkill == 0) {
				pc_useitem(sd,Item_Sp);
				}
			//}
		}
		if (Hp * 100 / MaxHp <= sd->state.Set_Flywing)
		{
			if (p.hotkey[8].isSkill == 0) {
				pc_useitem(sd, Item_flywing);
			}
		}
		...
		if (target_id) {
			...
			if (sd->class_ == MAPID_ARCH_BISHOP || sd->class_ == MAPID_ARCH_BISHOP_T || sd->class_ == MAPID_CARDINAL ) {
				if (!sd->sc.data[SC_BLESSING]) {
					unit_skilluse_id(&sd->bl, sd->bl.id, 2041, 3);//bless
				}
				//EFST_INC_AGI
				else if (!sd->sc.data[SC_INCREASEAGI]) {
					unit_skilluse_id(&sd->bl, sd->bl.id, 2042, 3);//agi
				}
				else if (!sd->sc.data[SC_MAGNIFICAT]) {
					unit_skilluse_id(&sd->bl, sd->bl.id, 74, 5);//mag
				}
				else if (!sd->sc.data[EFST_KYRIE]) {
			...
			if(sd->state.bCanAttack == 1)
			{
				unit_attack(&sd->bl, target_id, 1);
			}
			//lock target
			if (p.hotkey[0].isSkill ==1)
			{
				unit_skilluse_id(&sd->bl, target_id, p.hotkey[0].id, 10);
			}
			if (p.hotkey[1].isSkill == 1)
			{
				unit_skilluse_id(&sd->bl, target_id, p.hotkey[1].id, 10);
			}
			// AOE
			if (p.hotkey[2].isSkill == 1)
			{
				unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, p.hotkey[2].id, 10);
			}
			if (p.hotkey[3].isSkill == 1)
			{
				unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, p.hotkey[3].id, 10);
			}
			if (p.hotkey[4].isSkill == 1)
			{
				unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, p.hotkey[4].id, 10);
			}
			...
			if (rand() % 100 <= 15)
			{
				if (p.hotkey[5].isSkill == 1) {
					unit_skilluse_id(&sd->bl, sd->bl.id, p.hotkey[5].id, 10);
				}
			}
			if (rand() % 100 <= 15)
			{
				if (p.hotkey[6].isSkill == 1) {
					unit_skilluse_id(&sd->bl, sd->bl.id, p.hotkey[6].id, 10);
				}
			}
			if (rand() % 100 <= 15)
			{
				if (p.hotkey[7].isSkill == 1) {
					unit_skilluse_id(&sd->bl, sd->bl.id, p.hotkey[7].id, 10);
				}
			}
			...
			if (sd->class_ == MAPID_SWORDMAN || sd->class_ == MAPID_SWORDMAN_HIGH) {
				//unit_skilluse_id(&sd->bl, target_id, 10, 10);//bash
				unit_attack(&sd->bl, target_id, 1);
				if (p.hotkey[0].isSkill)
				{
					unit_skilluse_id(&sd->bl, target_id, p.hotkey[0].id, 10);
				}
			}
			//archer
			if (sd->class_ == MAPID_ARCHER || sd->class_ == MAPID_ARCHER_HIGH) {
				unit_skilluse_id(&sd->bl, target_id, 46, 10);//archer high
				unit_attack(&sd->bl, target_id, 1);
			}
			//aco
			if (sd->class_ == MAPID_ACOLYTE || sd->class_ == MAPID_ACOLYTE_HIGH) {
				unit_skilluse_id(&sd->bl, target_id, 156, 10);//holylight
				unit_attack(&sd->bl, target_id, 1);
			}
			//merchant
			if (sd->class_ == MAPID_MERCHANT || sd->class_ == MAPID_MERCHANT_HIGH) {
				unit_attack(&sd->bl, target_id, 1);
			}
			//thief
			if (sd->class_ == MAPID_THIEF || sd->class_ == MAPID_THIEF_HIGH) {
				unit_attack(&sd->bl, target_id, 1);
			}
			// mage - sage  MAPID_WIZARD || MAPID_HIGH_WIZARD || MAPID_PROFESSOR || MAPID_SORCERER_T || MAPID_WARLOCK_T || sd->class_ == MAPID_SAGE
			 if (sd->class_ == MAPID_MAGE || sd->class_ == MAPID_MAGE_HIGH  ) {
				unit_skilluse_id(&sd->bl, target_id, 13, 10);//soulstirke bolt
				unit_skilluse_id(&sd->bl, target_id, 14, 10);//cold bolt
				unit_skilluse_id(&sd->bl, target_id, 19, 10);//fire bolt
				//unit_skilluse_id(&sd->bl, target_id, 2204, 10);//fire bolt
				//unit_skilluse_id(&sd->bl, target_id, 26, 10);
				//unit_skilluse_id(&sd->bl, BL_MOB, 83, 5);//fire bolt
				//unit_skilluse_pos(&sd->bl,sd->bl.x, sd->bl.y, 83, 5);//meteo
			
				//unit_skilluse_id(&sd->bl, target_id, 83, 5);
			}
			 // --- C2
			 //HIGH Priest
			 if (sd->class_ == MAPID_PRIEST || sd->class_ == MAPID_HIGH_PRIEST) {
				 unit_skilluse_id(&sd->bl, target_id, 79, 10);//magnus
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 //HIGH Knight
			 if (sd->class_ == MAPID_KNIGHT || sd->class_ == MAPID_LORD_KNIGHT) {
				 unit_skilluse_id(&sd->bl, target_id, 62, 10);//bowlingbash
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 //HIGH Wizard
			 if (sd->class_ == MAPID_WIZARD || sd->class_ == MAPID_HIGH_WIZARD) {
				 unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 89, 5);//stormgust
				 unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 83, 5);//lord
				 unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 85, 5);//meteo
				 unit_attack(&sd->bl, target_id, 1);
				 
			 }
			 //WHITESMITH
			 if (sd->class_ == MAPID_BLACKSMITH || sd->class_ == MAPID_WHITESMITH) {
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 //sniper
			 if (sd->class_ == MAPID_HUNTER || sd->class_ == MAPID_SNIPER) {
				 unit_attack(&sd->bl, target_id, 1);
				 unit_skilluse_id(&sd->bl, target_id, 46, 10);
			 }
			 //asscorss
			 if (sd->class_ == MAPID_ASSASSIN || sd->class_ == MAPID_ASSASSIN_CROSS) {
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 //paladin
			 if (sd->class_ == MAPID_PALADIN || sd->class_ == MAPID_CRUSADER) {
				 unit_skilluse_id(&sd->bl, target_id, 251, 10);//shield boomerang
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 //champion
			 if (sd->class_ == MAPID_CHAMPION || sd->class_ == MAPID_MONK) {
				 unit_skilluse_id(&sd->bl, target_id, 267, 10);//finger
				 unit_attack(&sd->bl, target_id, 1);
				 if ((rand() % 100 <= 20)) {
					 unit_skilluse_id(&sd->bl, target_id, 401, 1);//zen
				 }
			 }
			 //sage
			 if (sd->class_ == MAPID_SAGE || sd->class_ == MAPID_PROFESSOR) {
				 unit_skilluse_id(&sd->bl, target_id, 13, 10);//soulstirke bolt
				 unit_skilluse_id(&sd->bl, target_id, 14, 10);//cold bolt
				 unit_skilluse_id(&sd->bl, target_id, 19, 10);//fire bolt
			 }
			 //rorge
			 if (sd->class_ == MAPID_ROGUE || sd->class_ == MAPID_STALKER) {
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 //alchemist
			 if (sd->class_ == MAPID_CREATOR || sd->class_ == MAPID_ALCHEMIST) {
				 unit_skilluse_id(&sd->bl, target_id, 230, 10);//acid
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 //bard-dancer
			 if (sd->class_ == MAPID_BARDDANCER || sd->class_ == MAPID_CLOWNGYPSY) {
				 unit_skilluse_id(&sd->bl, target_id, 394, 10);//arrowvalcan
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 // ------ C3
			 //warlock
			 if (sd->class_ == MAPID_WARLOCK_T || sd->class_ == MAPID_WARLOCK) {
				 unit_skilluse_id(&sd->bl, target_id, 2204, 10);//jackforst
				 unit_skilluse_id(&sd->bl, target_id, 2216, 10);//earth
			 }
			 //runeknight
			 if (sd->class_ == MAPID_RUNE_KNIGHT_T || sd->class_ == MAPID_RUNE_KNIGHT) {
				 unit_skilluse_id(&sd->bl, target_id, 2002, 2);//sonicwave
				 unit_attack(&sd->bl, target_id, 1);
			 }
			 // Ranger
			if(sd->class_ == MAPID_RANGER_T || sd->class_ == MAPID_RANGER) {
				unit_skilluse_id(&sd->bl, target_id, 2233, 10);//arrowstrom
				unit_attack(&sd->bl, target_id, 1);
				
			}
			// Bishop
			if (sd->class_ == MAPID_ARCH_BISHOP || sd->class_ == MAPID_ARCH_BISHOP_T) {
				if (!sd->sc.data[SC_BLESSING]) {
					unit_skilluse_id(&sd->bl, sd->bl.id, 2041, 3);//bless
				}
				//EFST_INC_AGI
				...
				if (!sd->sc.data[SC_MAGNIFICAT]) {
					unit_skilluse_id(&sd->bl, sd->bl.id, 74, 5);//mag
				}
				if (!sd->sc.data[EFST_KYRIE]) {
					unit_skilluse_id(&sd->bl, sd->bl.id, 2045, 10);//kyrie
				}
				if (rand() % 100 <= 10) {
					unit_skilluse_id(&sd->bl, sd->bl.id, 2043, 3);//heal
			...
			// mechanic
			if (sd->class_ == MAPID_MECHANIC || sd->class_ == MAPID_MECHANIC_T) {
				unit_skilluse_id(&sd->bl, target_id, 2280, 5);//axe tornado
				//unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 2280, 5);//axe tornado
				//unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 5006, 5);//lavaflow
				unit_attack(&sd->bl, target_id, 1);
			}
			// guilotincross
			if (sd->class_ == MAPID_GUILLOTINE_CROSS || sd->class_ == MAPID_GUILLOTINE_CROSS_T) {
				unit_skilluse_id(&sd->bl, target_id, 2022, 10);//crossimpact
				unit_skilluse_id(&sd->bl, target_id, 2036, 5);//rolling
				unit_attack(&sd->bl, target_id, 1);
			}
			// royalguard
			if (sd->class_ == MAPID_ROYAL_GUARD || sd->class_ == MAPID_ROYAL_GUARD_T) {
				unit_skilluse_id(&sd->bl, target_id, 2310, 10);//shield press
				unit_skilluse_id(&sd->bl, target_id, 2307, 10);//cannon spear
				//unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 2321, 10);//genesis ray
				unit_attack(&sd->bl, target_id, 1);
			}
			// sorcerer
			if (sd->class_ == MAPID_SORCERER || sd->class_ == MAPID_SORCERER_T) {
				unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 2447, 5);//pycho
				unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 2449, 5);//daimond
				unit_attack(&sd->bl, target_id, 1);
			}
			// minstel
			if (sd->class_ == MAPID_MINSTRELWANDERER || sd->class_ == MAPID_MINSTRELWANDERER_T) {
				unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 2418, 5);//arrow rain
				unit_attack(&sd->bl, target_id, 1);
			}
			// shura
			if (sd->class_ == MAPID_SURA || sd->class_ == MAPID_SURA_T) {
				unit_skilluse_id(&sd->bl, target_id, 2328, 10);//earth
				unit_skilluse_id(&sd->bl, target_id, 2326, 10);//dragoncombo
				//unit_attack(&sd->bl, target_id, 1);
			}
			// genetic
			if (sd->class_ == MAPID_GENETIC || sd->class_ == MAPID_GENETIC_T) {
				unit_skilluse_id(&sd->bl, target_id, 2476, 10);//cart tornado
				//unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 2476, 10);
				unit_attack(&sd->bl, target_id, 1);
			}
			// shadowcaster
			if (sd->class_ == MAPID_SHADOW_CHASER || sd->class_ == MAPID_SHADOW_CHASER_T) {
				unit_attack(&sd->bl, target_id, 1);
			}
			// gun
			if (sd->class_ == MAPID_GUNSLINGER) {
				unit_attack(&sd->bl, target_id, 1);
			}
			// ninja
			if (sd->class_ == MAPID_NINJA) {
				unit_skilluse_id(&sd->bl, target_id, 523, 5);//thorw shuriken
				unit_attack(&sd->bl, target_id, 1);
			}
			// tekon
			if (sd->class_ == MAPID_TAEKWON || sd->class_ == MAPID_STAR_GLADIATOR) {
				unit_skilluse_id(&sd->bl, target_id, 421, 5);//flying kick
				unit_attack(&sd->bl, target_id, 1);
			}
			//soulink
			...
			// NINJA2
			if (sd->class_ == MAPID_KAGEROUOBORO) {
				unit_skilluse_id(&sd->bl, target_id, 3009, 10);//thorw shuriken
				unit_attack(&sd->bl, target_id, 1);
				if ((rand() % 100 <= 20)) {
					unit_skilluse_id(&sd->bl, target_id, 3022, 1);//16th night
				}
			}
			//rebel
			
			if (sd->class_ == MAPID_REBELLION) {
			
				unit_skilluse_id(&sd->bl, target_id, 2565, 10);//round tip
				unit_attack(&sd->bl, target_id, 1);
			}
			//star emperor
			...
			//star emperor
			if (sd->class_ == MAPID_SOUL_REAPER) {
				unit_skilluse_id(&sd->bl, target_id, 2602, 10);//espa
				unit_skilluse_id(&sd->bl, target_id, 2604, 10);//eshwoo
				
				if ((rand() % 100 <= 20)) {
					unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 2609, 5);//soulcollect
					unit_skilluse_pos(&sd->bl, sd->bl.x, sd->bl.y, 2607, 5);//soulreaper
				}
				unit_attack(&sd->bl, target_id, 1);
			}*/

			    //unit_skilluse_id(&sd->bl, target_id,19,10);//48
	...
	if (!target_id && !pc_isdead(sd) && sd->state.autoattack)
	{
		if (sd->state.Move == 1)
		{
			...
			if (!target_id && (rand() % 100 <= 13)) {
				//pc_randomwarp(sd, CLR_TELEPORT);//telepot
				pc_useitem(sd, Item_flywing);
			}
		}
		else
...
{
	uint16 CanMove; // Canmove 0 notmove 1 move 2 disable
	uint16 Hp_per;
	uint16 Sp_per;
	uint16 flywing_per;
	uint16 CanAttack;
	if (!message || !*message || sscanf(message, "%6hu %6hu %6hu %6hu %6hu",&CanAttack,&CanMove,&Hp_per,&Sp_per,&flywing_per) < 1)
	{
		clif_displaymessage(fd, "Use : <CanAttack> 0-1 <Canmove> 0-2 <HP> 1-100 <SP> 1-100 <HpTele> 1-100");
		return -1;
	}
	nullpo_retr(-1, sd);
	if (sd->state.autoattack || CanAttack == 2)
	{
		sd->state.Move =0;
		sd->state.autoattack = 0;
		unit_stop_attack(&sd->bl);
		clif_displaymessage(fd, "Automatic : OFF.");
		sd->state.autoloot = 0;
	...
	else if (CanMove == 1)
	{
		//setting
		sd->state.Move = 1;
		sd->state.bCanAttack = CanAttack; // 0 not attack 1 attack
		sd->state.Set_Sp_Auto = Sp_per;
		sd->state.Set_Hp_Auto = Hp_per;
		sd->state.Set_Flywing = flywing_per;

		clif_hotkeys_send(sd,1);
		sd->state.autoattack = 1;
		add_timer(gettick() + 1000, autoattack_timer, sd->bl.id, 0);
		clif_displaymessage(fd, "Automatic : ON.");
		sd->state.autoloot = 8000;
